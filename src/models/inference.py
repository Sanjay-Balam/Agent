...
class ManimInferenceEngine:
    ...
    def __init__(self, model_path: str, tokenizer_path: str, device: str = "auto"):
        ...
        # Generation parameters (tuned for better code generation)
        self.default_generation_params = {
            'max_length': 256,  # Shorter for more focused scripts
            'temperature': 0.7,  # Lower temperature for more coherent code
    ...
    def generate_script(self, request: str, **generation_params) -> str:
        ...
        # Clean up script
        script = self._clean_generated_script(script, request)
    ...
    def _clean_generated_script(self, script: str, user_request: str = "") -> str:
        """Clean up the generated script."""
        # If the script looks malformed, try to reconstruct it
        if len(script.strip()) < 50 or script.count('class') > 2 or script.count('from manim import') > 2:
            return self._reconstruct_script(script, user_request)
        
        # Remove extra whitespaces but preserve line structure
        lines = script.split('\n')
        cleaned_lines = []
        ...
        script = script.replace(' ,', ',')
        script = script.replace(' :', ':')
        script = script.replace('import *', 'import *')
        
        # Try to identify and fix structure
        script = self._fix_structure(script, user_request)
        
        return script
    
    def _reconstruct_script(self, malformed_script: str, user_request: str = "") -> str:
        """Try to reconstruct a heavily malformed script."""
        # Try to extract intent from both the malformed script and user request
        script_lower = malformed_script.lower()
        request_lower = user_request.lower()
        
        # Generate appropriate template based on detected intent
        if 'sine' in script_lower or 'wave' in script_lower or 'sine' in request_lower or 'wave' in request_lower:
...
class SineWaveScene(Scene):
    def construct(self):
        ...
        elif 'circle' in script_lower or 'circle' in request_lower:
            template = '''from manim import *
...
class CircleScene(Scene):
    def construct(self):
        ...
        self.play(Rotate(circle, 2*PI))
        self.wait(1)'''
        
        elif 'square' in script_lower or 'rectangle' in script_lower or 'square' in request_lower or 'rectangle' in request_lower:
            template = '''from manim import *
...
class SquareScene(Scene):
    def construct(self):
        ...
        elif 'text' in script_lower or 'hello' in script_lower or 'text' in request_lower or 'hello' in request_lower:
            template = '''from manim import *
...
class TextScene(Scene):
    def construct(self):
        ...
        elif 'formula' in script_lower or 'equation' in script_lower or 'e=mc' in script_lower or 'formula' in request_lower or 'equation' in request_lower or 'e=mc' in request_lower:
            template = '''from manim import *
...
class FormulaScene(Scene):
    def construct(self):
        ...
        elif 'triangle' in script_lower or 'triangle' in request_lower:
            template = '''from manim import *
...
class TriangleScene(Scene):
    def construct(self):
        ...
        elif 'ball' in script_lower or 'bounce' in script_lower or 'ball' in request_lower or 'bounce' in request_lower:
            template = '''from manim import *

class BouncingBallScene(Scene):
    def construct(self):
        ball = Circle(radius=0.3, fill_opacity=1)
        ball.set_color(RED)
        ball.move_to(UP * 2)
        
        self.add(ball)
        self.play(ball.animate.move_to(DOWN * 2), rate_func=lambda t: abs(np.sin(PI * t)))
        self.wait(1)'''
...
class GeneratedScene(Scene):
    def construct(self):
        # Generated animation based on your request
        obj = Circle(radius=1)
        obj.set_color(BLUE)
    ...
    def _fix_structure(self, script: str, user_request: str = "") -> str:
        """Fix the structure of the script."""
        # Look for key patterns and reorder
        has_import = 'from manim import' in script or 'import' in script
        has_class = 'class' in script and 'Scene' in script
        has_def = 'def construct' in script
        has_self = 'self.play' in script or 'self.add' in script
        
        if not (has_class and has_def):
            return self._reconstruct_script(script, user_request)
        
        # Try to split and reorder components
        parts = script.split()
        
        # Look for import statement
        import_statement = 'from manim import *'
        if not has_import:
            script = import_statement + '\n\n' + script
        
        # Try to reconstruct proper Python structure
        lines = []
        current_line = []
        indent_level = 0
        ...
        while i < len(parts):
            ...
            if word == 'from' and i + 3 < len(parts) and parts[i+1] == 'manim' and parts[i+2] == 'import':
                if current_line:
                ...
                i += 3
                continue
            elif word == 'class' and i + 1 < len(parts):
                if current_line:
                    lines.append('    ' * indent_level + ' '.join(current_line))
                    current_line = []
                lines.append('')  # Empty line before class
                class_line = [word]
                i += 1
                while i < len(parts) and not parts[i].endswith(':'):
                    class_line.append(parts[i])
                    i += 1
                if i < len(parts):
                    class_line.append(parts[i])
                lines.append(' '.join(class_line))
                indent_level = 1
                i += 1
                continue
            elif word == 'def' and i + 1 < len(parts):
                if current_line:
                    lines.append('    ' * indent_level + ' '.join(current_line))
                    current_line = []
                def_line = [word]
                i += 1
                while i < len(parts) and not parts[i].endswith(':'):
                    def_line.append(parts[i])
                    i += 1
                if i < len(parts):
                    def_line.append(parts[i])
                lines.append('    ' + ' '.join(def_line))
                indent_level = 2
                i += 1
                continue
        ...
        result = result.replace('construct(self):', 'construct(self):')
