"""
Enhanced Multi-Domain Model Trainer
...
from multi_domain_knowledge_base import get_knowledge_base
...
class EnhancedModelTrainer:
    ...
    def plot_training_curves(self, save_dir: str) -> None:
        """Plot training curves."""
        ...
        try:
            ...
            # Domain accuracy
            plt.subplot(1, 3, 2)
            plt.plot(self.domain_accuracies, label='Domain Accuracy', color='green')
            plt.xlabel('Epoch')
            plt.ylabel('Accuracy')
            ...
            normalized_train = np.array(self.train_losses) / max(self.train_losses)
            normalized_val = np.array(self.val_losses) / max(self.val_losses)
            
            plt.plot(normalized_train, label='Train Loss (norm)')
            plt.plot(normalized_val, label='Val Loss (norm)')
            plt.plot(self.domain_accuracies, label='Domain Acc')
            plt.xlabel('Epoch')
...
def main():
    ...
    print("   3. Run: python3 fixed_api_server.py")
